...
資料存取層已改為優先連線 Supabase PostgreSQL，未設定 `SUPABASE_DB_URL` 時才使用本機 SQLite。最新設定方式見 `docs/12_supabase_setup.md`。
...
- 設定 Streamlit 頁面。
- 初始化 Supabase PostgreSQL 或本機 SQLite fallback。
- 初始化 session state。
- 判斷使用者是否登入。
- 根據側邊欄選單切換頁面。
...
用途：Supabase PostgreSQL / SQLite fallback 共用資料存取層。
